...
interface TrackedProof {
    ...
    Categoria: string;
    'Tempo Limite': string;
    Status: 'Em andamento' | 'Respondida';
}
                ...
                <div className="p-5 flex-shrink-0 border-b border-dark-border flex justify-between items-start">
                    <div className="flex items-center gap-4">
                        ...
                        <div className="min-w-0">
                            ...
                            <p className="text-sm text-gray-500 break-all">{result.email} | {result.telefone}</p>
                        </div>
                    </div>
                    <button onClick={onClose} className="text-gray-400 hover:text-white text-3xl font-bold">&times;</button>
                </div>
...
const ProofTrackingModal: React.FC<{
    ...
    const filteredProofs = useMemo(() => {
        ...
        if (filters.status) {
            tempProofs = tempProofs.filter(p => p.Status === filters.status);
        }
        ...
        for (const { categoryName, timeLimit } of selectedCategories) {
            const startPayload = {
                ...
                oral_test_data: {
                    ...
                    high_score_submissions: seller.highScores.map(score => ({
                        category: score.categoryName, questions_and_answers: score.detailedAnswers
                    }))
                }
            };
