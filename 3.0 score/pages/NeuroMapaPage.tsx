...
const NeuroMapaPage: React.FC = () => {
    ...
    const renderContent = () => {
        const companies = users.filter(u => u.role === UserRole.COMPANY && u.status === UserStatus.APPROVED);

        if (isLoading) {
            return <div className="text-center py-16"><p>Carregando dados do NeuroMapa...</p></div>;
        }
        ...
        switch (activeTab) {
            case 'manage':
                return <QuestionManager 
                            ...
                            onDeleteRequest={handleRequest}
                            onViewRequest={setViewingQuestion}
                        />;
            case 'view':
                return <QuestionViewer questions={neuroQuestions} categories={neuroCategories} isLoading={isLoading} onViewRequest={setViewingQuestion} />;
            case 'categories':
            ...
            case 'company_results':
                return (
                    <div>
                        <div className="mb-6 p-4 bg-dark-card rounded-xl border border-dark-border">
                            <label htmlFor="company-filter" className="block text-sm font-medium text-gray-300 mb-1">Filtrar por Empresa</label>
                            <select 
                                id="company-filter" 
                                value={companyFilter} 
                            ...
                            >
                                ...
                                {companyResultCompanies.map(company => (
                                    <option key={company} value={company}>{company}</option>
                                ))}
                            </select>
                        </div>
                        <ResultsPanel
                            title="Resultados - Empresas"
            ...
            case 'employee_results':
                return (
                     <div>
                        <div className="flex flex-col sm:flex-row gap-4 mb-6 p-4 bg-dark-card rounded-xl border border-dark-border">
                            <div className="flex-1">
                                <label htmlFor="employee-company-filter" className="block text-sm font-medium text-gray-300 mb-1">Filtrar por Empresa</label>
                                <select 
                                ...
                                >
                                    <option value="all">Todas as Empresas</option>
                                    {employeeResultCompanies.map(company => (
                                        <option key={company} value={company}>{company}</option>
                                    ))}
                                </select>
                            </div>
                            <div className="flex-1">
                                <label htmlFor="employee-name-filter" className="block text-sm font-medium text-gray-300 mb-1">Filtrar por Funcionário</label>
                                <select 
                                ...
                                >
                                    ...
                                    {employeesInSelectedCompany.map(name => (
    ...
    const FullScreenLoader = () => (
        <div className="fixed inset-0 bg-dark-background bg-opacity-90 z-[60] flex flex-col justify-center items-center p-4">
            <div className="loader-container">
    ...
    return (
        <div className="container mx-auto">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
                 <h1 className="text-2xl sm:text-3xl font-bold">NeuroMapa</h1>
            </div>

            <div className="mb-8 flex flex-wrap border-b border-dark-border">
                <button
                    onClick={() => setActiveTab('manage')}
                    className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === 'manage' ? 'border-b-2 border-cyan-400 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
                >
                    Gerenciar Perguntas
                </button>
                 <button
                    onClick={() => setActiveTab('view')}
                    className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === 'view' ? 'border-b-2 border-cyan-400 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
                >
                    Visualizar Perguntas
                </button>
                <button
                    onClick={() => setActiveTab('categories')}
                    className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === 'categories' ? 'border-b-2 border-cyan-400 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
                >
                    Gerenciar Categorias
                </button>
                <button
                    onClick={() => setActiveTab('company_results')}
                    className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === 'company_results' ? 'border-b-2 border-cyan-400 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
                >
                    Resultados (Empresas)
                </button>
                 <button
                    onClick={() => setActiveTab('employee_results')}
                    className={`px-4 py-2 text-sm font-medium transition-colors ${activeTab === 'employee_results' ? 'border-b-2 border-cyan-400 text-cyan-400' : 'text-gray-400 hover:text-white'}`}
                >
                    Resultados (Funcionários)
                </button>
            </div>
